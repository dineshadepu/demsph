...
use granules::contact_search::{get_neighbours_ll, LinkedListGrid};
...
fn test_grid_cells_indices() {
    ...
    let nbrs = get_neighbours_ll([ent1.x[0], ent1.y[0], 0.], &grid, &2);
    assert_eq!(0, nbrs.len());
    let nbrs = get_neighbours_ll([ent1.x[0], ent1.y[0], 0.], &grid, &1);
...
fn test_grid_cells_indices_a_corner_case() {
    ...
    ent1.h = array![1., 1., 1.];
    ent1.x = array![0., 0.1, 0.];
    ent1.y = array![0., 0., 0.1];
    let mut ent2 = DemDiscrete::new(3, 2, String::from("ent2"));
    ent2.h = array![1., 1., 1.];
    ent2.x = array![-0.1, 0., 0.1];
    ent2.y = array![0., -0.1, 0.1];
