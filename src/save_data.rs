...
impl DumpData for WCSPH {
    fn save_data(&self, output_folder_name: &str, time_step_number: usize) {
        ...
        writeln!(&mut file, "SCALARS hdx float 1").unwrap();
        writeln!(&mut file, "LOOKUP_TABLE default").unwrap();
        for i in 0..np {
            writeln!(&mut file, "{}", self.h[i]).unwrap();
        }
        // write mass
        ...
        for i in 0..np {
            writeln!(&mut file, "{} {} {}", self.ax[i], self.ay[i], self.az[i]).unwrap();
        }
    }
    ...
    fn write_vtk_xml(&self, output_folder_name: &str, time_step_number: usize) {
        ...
        writeln!(&mut file, "VECTORS Force float").unwrap();
        for i in 0..np {
            writeln!(&mut file, "{} {} 0.0", self.ax[i], self.ay[i]).unwrap();
        }
    }
}
